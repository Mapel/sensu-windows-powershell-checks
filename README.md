...
Attention: as I learned there is a problem with the exit code (see https://stackoverflow.com/a/60774177/22537111):
...
If you want to use -Command that you have to put
```
; exit $LASTEXITCODE
```
behind it.
Or you use -File. In that case you have to use the environment variable for the dynamic asset path. (Check https://docs.sensu.io/sensu-go/latest/plugins/assets/#dynamic-runtime-asset-path) In my case it looks like this:
```
Powershell.exe -NonInteractive -NoProfile -ExecutionPolicy Bypass -NoLogo -File %SENSU_WINDOWS_POWERSHELL_CHECKS_PATH%\bin\check-test.ps1 -Critical
```
